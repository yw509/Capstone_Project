...
def rank_bm25_retrieve(policy_domain_idx, policy_domain, eval_data_chunk, train_data_chunk, evaluation_model, similar_item, output_dir, remove_duplicate_titles, exp_name = None):
    ...
    for _, eval_row in tqdm(eval_data_chunk.iterrows(), desc='Evaluating' if not exp_name else exp_name):
        ...
        # Prepare the target advertisement
        target_ad_dict = {
            'Target Advertisement': pd.DataFrame([{
                'title': eval_row['title'],
                'description': eval_row['description'],
                'thumbnail_url': eval_row['thumbnail_url']
            }])
        }
        ...
        target_content = convert_dataset_to_user_prompt(
            ...
            included_comment_col=None,  # No comments for target ad
            img_detail_level='low'
        )

        # Prepare example advertisements
        example_ads_dict = {
            'Example Advertisements': pd.concat([
                pd.DataFrame(top_approved).assign(label="Compliant Ads"),
                pd.DataFrame(top_rejected).assign(label="Non-Compliant Ads")
            ])
        }
        ...
        examples_content = convert_dataset_to_user_prompt(
            example_ads_dict,
            components_override=[
                'thumbnail_url', 'title', 'description', 'approval_label',
